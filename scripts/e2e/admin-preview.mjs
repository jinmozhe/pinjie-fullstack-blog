...
export function startAdminPreview() {
  ...
  const backend = new URL(process.env.E2E_BACKEND_URL ?? "http://127.0.0.1:8100");
  if (backend.href !== "http://127.0.0.1:8100/") {
    throw new Error("Production Admin preview requires the local Backend on 127.0.0.1:8100.");
  }
  const name = `pinjie-admin-preview-${randomUUID()}`;
  const network = process.platform === "linux"
    ? ["--network", "host", "--add-host", "backend:127.0.0.1"]
    : ["--publish", "127.0.0.1:3101:3101", "--add-host", "backend:host-gateway"];
