...
if (process.argv[2] === "cleanup") {
  ...
  try {
    ...
    const environment = {
      ...
      WEB_ORIGINS: '["http://127.0.0.1:3100"]', ADMIN_ORIGINS: '["http://127.0.0.1:3101"]',
    ...
    const config = {
      services: {
        ...
        backend: { image: request.images.backend.release.image.reference, platform: "linux/amd64", environment,
          ports: ["127.0.0.1:8100:8100"], depends_on: { postgres: { condition: "service_healthy" }, redis: { condition: "service_healthy" } } },
        web: { image: request.images.web.release.image.reference, platform: "linux/amd64", ports: ["127.0.0.1:3100:3100"],
          environment: { BACKEND_INTERNAL_URL: "http://backend:8100", WEB_PUBLIC_ORIGIN: "http://127.0.0.1:3100" },
          depends_on: { backend: { condition: "service_healthy" } } },
        admin: { image: request.images.admin.release.image.reference, platform: "linux/amd64", ports: ["127.0.0.1:3101:3101"],
          depends_on: { backend: { condition: "service_healthy" } } },
      },
    };
