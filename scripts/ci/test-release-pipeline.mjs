...
const request = { schema: "pinjie-candidate-request-v1", test_commit: sha, web_public_origin: "https://www.example.com",
  images: Object.fromEntries(apps.map((app, index) => [app, { handoff_run_id: "123", release: {
    ...
    registry: "ccr.ccs.tencentyun.com", namespace: "pinjie-fullstack-blog",
    cnb: { repository: "pjwl/pinjie-fullstack-blog", branch: "main", pipeline: `${app}-image`, build_id: "88",
      build_url: "https://cnb.cool/pjwl/pinjie-fullstack-blog/-/build/88", started_at: source.commit_time, finished_at: source.commit_time },
    image: { reference: `ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-blog-${app}@sha256:${String(index + 1).repeat(64)}`,
      repository: `pinjie-blog-${app}`, trivy: "passed", sbom: "cyclonedx-json", provenance: "buildkit-max",
      oci: { revision: sha, created: source.commit_time, source: "https://github.com/jinmozhe/pinjie-fullstack-blog" },
...
for (const mode of ["success", "source-error", "wrong-digest", "stale-app", "browser-error", "partial-browser", "cleanup-error"]) {
  ...
  try {
    const result = spawnSync(process.execPath, ["--import", "./scripts/ci/fixtures/candidate-processes.mjs", "scripts/release/candidate-images.mjs", "validate"], {
      ...
      env: { ...process.env, GITHUB_ACTIONS: "true", GITHUB_REPOSITORY: "jinmozhe/pinjie-fullstack-blog", GITHUB_REF: "refs/heads/main",
