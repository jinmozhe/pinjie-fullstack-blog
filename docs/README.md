...
| 文件                                               | 说明                                                             |
| -------------------------------------------------- | ---------------------------------------------------------------- |
| [PROJECT_REQUIREMENTS.md](PROJECT_REQUIREMENTS.md) | 独立博客的用户、首版需求、非目标和验收标准 |

---

## adr/ - 架构决策记录

> ADR 0001 至 0015 为继承的工程决策；博客适用边界由 ADR 0016 明确。历史决策只追加替代说明，修复失效引用不改变原决策。历史生产地址与验证数字不作为博客当前证据。
...
| [0016-博客独立工程与维护边界决策.md](adr/0016-博客独立工程与维护边界决策.md) | 独立 Git、环境和发布边界，取消母版追踪并重建博客计划基线 |
...
> 以下为继承的业务参考资料，不属于博客首版需求或已实现能力。博客功能以产品需求基线和首版全栈计划为准。
...
| [blog-writing.md](operations/blog-writing.md) | 已有本地环境启用文章后台的迁移、权限同步、启动和写作操作 |
