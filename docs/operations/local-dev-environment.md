...
> 适用仓库：独立博客 `pinjie-fullstack-blog`
...
```text
Windows 本机
├── Backend：uv + 标准 CPython 3.14 + 项目 .venv，localhost:8000
├── Web：pnpm + Next.js，localhost:3000
├── Admin：pnpm + Umi Max，localhost:3001
└── PostgreSQL：本机服务，端口 5432

Docker Desktop
└── Redis 8.10.0：根目录 compose.yml，宿主端口 6381（容器内 6379）
```

Anaconda 或 Miniconda 不属于本项目的前置依赖。日常后端命令统一使用 `uv sync` 和 `uv run`，不要在主流程中混用 Conda 环境。

博客 Compose 使用独立项目名 `pinjie-fullstack-blog-local`，不复用其他项目 Redis 容器或卷。Web/Admin 默认使用 `localhost`；地址必须与 Backend 的精确 Origin 配置一致。同一时刻启动多个本地项目时，端口不能重复绑定，需要停止不使用的项目或显式调整端口与 Origin。

## 二、本地与生产环境区别

博客尚未配置生产目标。生产保持容器内部端口 Backend 8100、Web 3100、Admin 80；不同 Compose 项目可以复用内部端口。同一服务器的宿主映射端口必须逐项目分配，不得冲突，OpenResty 域名反代填写各项目的宿主映射地址。本地 8000/3000/3001 不会自动改变生产映射。

以下是保留的 Linux x86_64 / 1Panel 部署参考拓扑，其中 8100/3100/3101 是该项目示例宿主映射端口，实际部署必须按博客发布计划重新核对：

```text
1Panel
├── OpenResty
│   ├── api.yourdomain.com   → 127.0.0.1:8100
│   ├── admin.yourdomain.com → 127.0.0.1:3101
│   └── www.yourdomain.com   → 127.0.0.1:3100
...
| Redis | Docker Desktop，`127.0.0.1:6381` | 1Panel 共享 Redis 8.10.0，通过外部网络访问 |
...
以下命令默认从仓库根目录 `pinjie-fullstack-blog` 开始执行。
...
- 按现有 `.python-version` 安装标准 CPython，不修改锁定版本，不使用 free-threaded `3.14t`。
...
```sql
CREATE USER pinjie_blog WITH PASSWORD 'your_local_password';
CREATE DATABASE pinjie_blog_dev OWNER pinjie_blog;
CREATE USER pinjie_blog_test WITH PASSWORD 'your_test_password' CREATEDB;
CREATE DATABASE pinjie_blog_test OWNER pinjie_blog_test;
GRANT ALL PRIVILEGES ON DATABASE pinjie_blog_dev TO pinjie_blog;
GRANT ALL PRIVILEGES ON DATABASE pinjie_blog_test TO pinjie_blog_test;
```
...
```dotenv
DATABASE_URL=postgresql+asyncpg://pinjie_blog:your_local_password@localhost:5432/pinjie_blog_dev
TEST_DATABASE_URL=postgresql+asyncpg://pinjie_blog_test:your_test_password@localhost:5432/pinjie_blog_test
REDIS_URL=redis://127.0.0.1:6381/0
TEST_REDIS_URL=redis://127.0.0.1:6381/15
```

前端公开变量会进入浏览器构建产物，禁止在 `NEXT_PUBLIC_` 或 `VITE_` 变量中保存密钥。

应用配置与四个独立认证密钥必须按[环境变量手册](environment-variables-and-backend-local-run.md)填写。运行前对新建的 `pinjie_blog_dev` 执行经确认的 Alembic 升级、权限同步和初始管理员创建；不要在每次启动时自动迁移。测试库创建、迁移及测试命令不在日常自动执行范围。
...
```powershell
pg_dump -U pinjie_blog -d pinjie_blog_dev -t categories --data-only --file seed.sql
```
...
```sql
CREATE USER pinjie_blog WITH PASSWORD 'strong_production_password';
CREATE DATABASE pinjie_blog_prod OWNER pinjie_blog;
GRANT ALL PRIVILEGES ON DATABASE pinjie_blog_prod TO pinjie_blog;
```
