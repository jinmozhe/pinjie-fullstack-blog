...
```text
https://cnb.cool/pjwl/pinjie-fullstack-blog-secrets/-/blob/main/tcr-publish.yml
```
...
TCR_NAMESPACE: "pinjie-fullstack-blog"
TCR_PUBLISH_USERNAME: "<TCR 发布用户名>"
TCR_PUBLISH_PASSWORD: "<TCR 固定密码>"

allow_slugs:
  - "pjwl/pinjie-fullstack-blog"
allow_events:
  - "push"
...
`buildcache-main` 是可变构建缓存，`candidate-<CNB Build ID>` 是单次运行候选，两者都不能作为部署来源。生产只使用发布清单中的完整 `ccr.ccs.tencentyun.com/pinjie-fullstack-blog/<镜像>@sha256:<digest>` 引用。
...
```dotenv
BACKEND_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-backend@sha256:<64位十六进制摘要>
WEB_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-web@sha256:<64位十六进制摘要>
ADMIN_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-admin@sha256:<64位十六进制摘要>
...
DATABASE_URL=postgresql+asyncpg://pinjie_blog_app:<URL编码后的生产密钥>@postgresql:5432/pinjie_blog_prod
REDIS_MODE=required
...
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.sync_permissions --apply --confirm-database pinjie_blog_prod
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.sync_permissions --check --confirm-database pinjie_blog_prod
```

首次创建超级管理员时单独执行交互命令：

```powershell
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.create_initial_admin --username initial-admin --confirm-database pinjie_blog_prod
```
...
```powershell
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.cleanup_security_logs --confirm-database pinjie_blog_prod
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.cleanup_security_logs --apply --confirm-database pinjie_blog_prod
```

升级到会话设备名称识别版本后，可以先预览旧会话回填数量；核对目标数据库并取得写入授权后再增加 `--apply`：

```powershell
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.backfill_session_device_names --confirm-database pinjie_blog_prod
docker compose --env-file .env -f compose.prod.yml run --rm backend python -m scripts.backfill_session_device_names --apply --confirm-database pinjie_blog_prod
```
