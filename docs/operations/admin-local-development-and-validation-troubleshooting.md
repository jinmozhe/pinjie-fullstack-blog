...
> 相关计划：Admin 升级 Ant Design 6 计划（继承计划已由用户清理）
...
开发服务默认只监听 `http://localhost:3001`。`apps/admin/scripts/run-umi.mjs` 注入 `HOST=localhost` 和 `PORT=3001`，Umi Webpack 精确补丁确保底层 `listen` 调用实际使用该 host。需要局域网访问时必须显式设置 `HOST`，并先评估开发服务暴露风险。直接调用 Umi 时，工作目录必须是 `apps/admin` 且显式设置 `HOST=localhost`，避免绕过端口和监听边界。
...
4. E2E 专用 Backend 已完成测试库迁移并监听 `8100`；日常开发使用 `localhost:8000`，不混用环境。
