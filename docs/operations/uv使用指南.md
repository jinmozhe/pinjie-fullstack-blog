...
>
> 本文档说明本项目（pinjie-fullstack-blog）为何选择「纯 uv 模式」，
...
│       └── ...
│
├── pinjie-fullstack-blog/apps/backend/
│   └── .venv/                           ← 项目私有虚拟环境
...
```text
项目 A：pinjie-fullstack-blog  → 标准 CPython 3.14，依赖版本由 uv.lock 决定
...
| 启动开发服务器 | `uv run uvicorn app.main:app --reload --port 8100` | `apps/backend/` |
