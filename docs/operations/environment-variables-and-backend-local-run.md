...
博客尚未配置生产域名、镜像或部署主机。下面生产片段只描述配置结构；发布入口当前明确失败。开发数据库为 pinjie_blog_dev，测试数据库为 pinjie_blog_test，不能连接母版数据库。
...
```text
C:\path\to\pinjie-fullstack-blog
```

Backend 项目目录为：

```text
C:\path\to\pinjie-fullstack-blog\apps\backend
```
...
根 `.env` 是部署控制文件，不是某个应用容器的通用运行配置。目标配置应保存三张博客镜像的完整不可变引用和 Web 公开 Origin，以下仅为命名示例：

```dotenv
BACKEND_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-blog-backend@sha256:<64位十六进制摘要>
WEB_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-blog-web@sha256:<64位十六进制摘要>
ADMIN_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-blog-admin@sha256:<64位十六进制摘要>
...
Web 生产容器通过 Compose 的 `BACKEND_INTERNAL_URL=http://backend:8100` 连接 Backend，浏览器仍访问同域 `/api/v1`。

### 3.4 Admin `.env.local`

Admin 使用 Umi Max，`VITE_*` 变量会进入浏览器可读的静态 JavaScript，主要在构建阶段由 `config/config.ts` 注入。修改已构建容器中的 `.env.local` 通常不能改变现有静态产物。

Admin 使用同域相对路径 `/api/v1`，开发服务器和生产 Nginx 都代理到 Backend。Admin 本地或 CI 开发服务器可通过 `BACKEND_INTERNAL_URL` 指定 Backend 地址，默认使用 `http://localhost:8000`；代理保留浏览器 `Origin`，以满足 Backend 的严格来源校验。日常浏览器统一使用 localhost，避免与 127.0.0.1 混用造成 Cookie 和来源校验失败。
...
```powershell
Set-Location C:\path\to\pinjie-fullstack-blog
...
```powershell
Set-Location C:\path\to\pinjie-fullstack-blog\apps\backend
```
...
ENVIRONMENT=local
DEBUG=true
DATABASE_URL=postgresql+asyncpg://pinjie_blog:<本地密码>@localhost:5432/pinjie_blog_dev
...
首次建库或有新增迁移、权限时，先核对 `.env` 连接博客自己的数据库并做好必要备份，再在 Backend 目录执行：
...
4. 同步后再次 `--check`，确认没有权限目录漂移；不必重新创建已有管理员。博客第二阶段的具体启用步骤见 [博客后台启用与写作](blog-writing.md)。

日常启动只执行以下命令，不自动迁移或同步权限：
...
```powershell
uv run python -m scripts.create_initial_admin --username initial-admin --confirm-database pinjie_blog_dev
```
...
集成测试同时使用 `TEST_REDIS_URL=redis://127.0.0.1:6381/15` 隔离 Redis 数据；测试 DB 15 不得与开发用途混用。
...
uv run python -m scripts.verify_local_database_recovery `
  ...
  --confirm-source-database pinjie_blog_test `
...
# 查看超过保留期的记录数量，不删除数据
uv run python -m scripts.cleanup_security_logs --confirm-database pinjie_blog_dev

# 经审批后应用保留期清理
uv run python -m scripts.cleanup_security_logs --apply --confirm-database pinjie_blog_dev

# 预览旧会话设备名称回填，不写数据库
uv run python -m scripts.backfill_session_device_names --confirm-database pinjie_blog_dev

# 核对数量并取得数据库写入授权后应用回填
uv run python -m scripts.backfill_session_device_names --apply --confirm-database pinjie_blog_dev
```
...
```text
C:\path\to\pinjie-fullstack-blog\apps\backend\.venv\Scripts\python.exe
```
