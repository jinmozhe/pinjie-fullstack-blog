...
本文面向使用 AI 助手维护 `pinjie-fullstack-blog` 的项目负责人、开发者和评审者，完整说明：
...
```text
E:\fastapi\pinjie-fullstack-blog
```
...
| --- | --- |
| 是否自动加载 | 否 |
| 必须读取 | 新增、修改、取消产品能力；创建实现计划；判断博客产品范围 |
| 条件读取 | 评审实现是否满足需求编号或验收条件 |
| 通常无需读取 | 不改变产品能力的局部修复或只运行既有检查 |
| 主要内容 | 目标用户、适用场景、目标能力、非目标和验收边界 |
...
| 博客工程独立化 | 根规则、PRD、`PROJECT_INDEX.md` | ADR 0016、实际配置与 Git 状态 | 必须 | 仅博客 origin、独立数据和 Cookie；按现存计划重建索引，不要求母版 Tag/SHA |
...
如果问题只涉及“博客是否应该提供某项能力”，可以不读取完整 Backend 工程标准。讨论已经深入到 Router、事务、Schema、迁移或测试写法时，应继续读取工程标准。
...
历史 ACL 实施证据保留在 Codex Windows ACL 长期治理计划（继承计划已由用户清理），标准文档的建立记录保留在 Codex Windows 配置与 ACL 标准文档计划（继承计划已由用户清理），当前默认联网和 Schannel 边界记录保留在 Codex Windows 网络与 Schannel 边界治理计划（继承计划已由用户清理）。
