...
```text
/home/ubuntu/projects/pinjie-fullstack-blog/
├── compose.prod.yml
├── .env
└── apps/
    └── backend/
...
打开 CNB 仓库 `pjwl/pinjie-fullstack-blog` 的构建记录。`main` 收到 GitHub 交接后，根据实际变更路径自动触发：
...
```text
ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-backend@sha256:<64位摘要>
ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-web@sha256:<64位摘要>
ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-admin@sha256:<64位摘要>
```
...
```dotenv
BACKEND_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-backend@sha256:<64位摘要>
WEB_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-web@sha256:<64位摘要>
ADMIN_IMAGE=ccr.ccs.tencentyun.com/pinjie-fullstack-blog/pinjie-fullstack-admin@sha256:<64位摘要>
...
```bash
cd /home/ubuntu/projects/pinjie-fullstack-blog
...
7. 在“容器 > 编排”打开 `pinjie-fullstack-blog`，点击“保存”或“更新编排”。
...
1Panel 保存整个编排时会重新计算全部服务。配置和镜像引用没有变化的服务通常保持运行，镜像 digest 变化的服务会创建新容器并替换旧容器。容器名称继续使用 `pinjie-fullstack-blog-<service>-1`，名称不随镜像 digest 改变。
