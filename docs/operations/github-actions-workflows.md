...
2. 校验 CNB 仓库 URL 固定为 `https://cnb.cool/pjwl/pinjie-fullstack-blog`，目标分支固定为 `main`，HTTPS 用户名固定为 `cnb`，Token 非空。
...
CNB `.cnb.yml` 声明三个具名 `main.push` Pipeline，并提供受控的 `main.web_trigger_full_release` 人工全量入口。每条 Pipeline 使用 4 核 Linux AMD64 社区构建节点、固定 digest 的构建环境、独立 Docker 配置目录和按应用划分的 TCR 发布锁。CNB 密钥仓库文件只允许 `pjwl/pinjie-fullstack-blog` 的 `main` Push 与 `web_trigger_full_release` 引用，并提供 TCR Registry 登录所需参数。
