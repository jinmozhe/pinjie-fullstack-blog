...
- 标准管理列表页复用 `src/components/PageFrame.tsx`，统一接入 `workspace-page` 与 `workspace-panel`。母版当前接口要求传入 description；页面没有说明需求时，不为满足展示形式增加空区块或间距占位，需要调整可选性时在后续实现中同步组件类型与调用方。
...
- 空数据与筛选无结果时，不得沿用有数据宽表的固定 `scroll.x` 和整组固定内容列宽，制造不必要的横向滚动条。按当前页实际 `items.length` 判断，空表取消固定滚动宽度、释放内容列宽并采用 `tableLayout="auto"`，操作列继续保持 `width: "1%"`；有数据时恢复该列表所需的列宽与滚动配置。窄屏表头确实无法容纳时仍在表格内部滚动，不隐藏表头、强制换行或用 `overflow: hidden` 裁掉内容。
...
现有用户、管理员和角色列表的状态 Tag 沿用已交付交互；本节规定后续新增或明确调整状态交互时的实现方式，不代表母版已完成 Switch 源码迁移。
...
| 具体业务依赖 | 编辑器、图表、Excel 等外围能力 | 只按博客已确认需求和计划引入，遵守兼容、锁定与验证规则 |
