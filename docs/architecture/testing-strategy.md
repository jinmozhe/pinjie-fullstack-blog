...
- E2E 默认针对生产构建运行：Web 使用 `next build` 后的 standalone server，Admin 使用 `max build`，开发/预览通过 `apps/admin/scripts/run-umi.mjs` 设置 `PORT=3101`。Web 构建脚本同时把静态资源准备到 standalone 目录。
...
博客尚未执行继承测试套件的真实回归。继承实现中的阶段 B/C 验证数字不作为博客验收证据；当前验证结果只记录在博客首版全栈计划。
