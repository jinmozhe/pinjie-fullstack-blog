...
本文说明本博客统一文件资产能力的领域边界、存储流程、安全策略和一致性语义。当前实现提供本地磁盘驱动；后续云对象存储扩展须保持同一 `StorageProvider` 契约。
...
文章正文与封面引用保存在 `post_assets`。资产单条及批量删除在锁定目标后检查文章引用，公开、隐藏和回收站中的文章均受保护；被引用时返回 409，不进入文件暂存删除。文章保存与资产删除使用同一资产行锁和数据库外键约束，避免保存成功后图片被并发删除。

文章隐藏、软删除不解除图片引用，也不撤销已知静态图片直链。未提交文章的图片上传独立于文章发布，放弃写作后的未引用资产由管理员人工清理。详细机制见 [博客内容架构](blog-content.md)。
