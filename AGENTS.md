...
- 本仓库是独立个人博客，技术栈为 FastAPI、Next.js、React、pnpm 和 Turborepo，面向一位管理员和匿名读者。
- 博客业务直接在本仓库实现。只配置博客自己的 `origin`，不配置母版远端、不跟踪母版 Tag/SHA、不同步上游、不依赖母版本机目录；内部 workspace 包继续由本仓库维护。
...
- `docs/PROJECT_REQUIREMENTS.md` 是博客目标用户、首版能力、非目标和验收边界的权威来源。
- 新增、修改或取消产品能力前必须读取产品需求基线，并在全栈计划中列出关联的 `BLOG-*` 需求编号。
...
- 博客不要求母版基线登记或升级同步。用户人工清理历史计划后，只按现存文件同步登记；全部现存及新增计划继续永久保护。
- 继承 ADR 的历史决策由博客 ADR 0016 明确适用范围。继承发布记录、蓝图和测试数字不证明博客已发布、已实现或已通过验收。
- 发布目标未配置时必须明确失败，不得调用母版 CNB Secrets、镜像仓库或生产环境。当前可执行部署边界见 `docs/operations/release-and-rollback.md`。
...
- 正式发布、生产部署、安全审计、故障回滚和阶段性交接属于跨系统追溯场景，必须在对应记录中保存完整 40 位 Commit SHA 或受保护的不可变 Git Tag；部署、审计和回滚记录优先使用完整 SHA。
