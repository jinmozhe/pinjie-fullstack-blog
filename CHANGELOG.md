...
- 完成第三阶段公开阅读源码：首页、分类页、标签页、文章详情及标题摘要搜索；统一 public 且未删除过滤、匿名 DTO、生成 SDK、按请求服务端读取、独立 Metadata/canonical、文章 JSON-LD 与分片站点地图。Web 已接通 Tailwind 3、CSS 变量、shadcn 本地基础组件、Radix 与 Lucide，关闭读者页面与公开注册，保留后台认证。数据库迁移、业务测试、构建及浏览器验收未执行，尚未发布部署。
- 建立 [Web 统一设计标准](docs/architecture/web-design-standard.md)，集中维护配色、字体、间距、组件、响应式、正文排版与可访问性规则，并由 Web AGENTS 强制引用。同步文档入口及首版标题摘要搜索需求，随后第三阶段实现已按该标准接入。
- 精确修复 Markdown 检查工具的 smol-toml 间接依赖，升级至 1.7.1 以处理高危拒绝服务漏洞 GHSA-7w5x-hrqm-74c2，保留 markdownlint-cli2 版本与规则。
- 文章日期筛选使用 ProComponents 的单个填充式日期范围控件，移除上方标签和应用侧 dayjs 直接依赖，保留中文起止日提示、无障碍名称及原日期筛选语义。
- 调整 Admin 文章列表列宽：标题占用剩余空间，分类与标签按比例分配，状态与时间按单行内容收紧；取消有数据时强制 1050px 宽度，操作列保持原规则。
- 修复 Admin 文章、分类、标签空数据时固定表格宽度引起的多余横向滚动，空表使用弹性列宽，有数据时保留宽表滚动；补充 Admin 空表布局规则。
- 完成第二阶段后台写作源码：文章发布、编辑、公开隐藏、软删除恢复、分类标签、Markdown 安全预览、图片上传插入与引用删除保护。新增五表迁移、13 项权限和生成契约；保留已有认证、设置和通用页面。
- 本地开发统一 localhost:8000/3001/3000，生产保留内部端口并按项目分配宿主映射；同步图片直链策略、后台启用手册与内容架构。三端轻量检查、契约漂移与文档治理通过；迁移、权限同步、重型测试与浏览器验收未执行，公开阅读留在第三阶段。
- 建立博客产品需求、独立维护决策和首版全栈计划，按用户人工清理结果重建计划登记。
- 分离项目身份、Cookie、三端端口、本地 Redis 和部署资源名称；保留迁移及通用基础能力。
- 博客发布目标尚未配置，GitHub 与 CNB 发布入口明确阻断。
- 已重建无提交的独立 main，仅保留博客 origin；旧 Git 完整备份到仓库外。三端轻量门禁、文档治理与契约生成通过，真实环境按用户要求后续配置，未执行重型验证、提交推送或发布部署。

## 继承代码历史参考

以下为代码起点附带的历史记录，保留原文以说明实现来源，不属于博客发布历史，其测试、镜像和部署结果不作为博客验收证据。
