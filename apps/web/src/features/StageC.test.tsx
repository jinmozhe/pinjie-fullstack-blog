...
describe("stage C web account", () => {
  ...
  it("redirects to login when SSR account session recovery fails", async () => {
    server.use(
      http.post("http://127.0.0.1:3100/api/v1/auth/refresh", () =>
  ...
  it("reads enabled, disabled, and unavailable public registration states", async () => {
    ...
    process.env.BACKEND_INTERNAL_URL = "http://127.0.0.1:3100";
    try {
      expect(await fetchRegistrationState()).toBe("enabled");
      server.use(
        http.get("http://127.0.0.1:3100/api/v1/system/capabilities", () => HttpResponse.json({
          code: "OK",
          message: "操作成功",
  ...
  it("shows the backend message and retry delay when registration is rate limited", async () => {
    server.use(
      http.post("http://127.0.0.1:3100/api/v1/auth/register", () => HttpResponse.json(
  ...
  it("shows profile update failures without losing the form", async () => {
    server.use(
      http.patch("http://127.0.0.1:3100/api/v1/users/me", () => HttpResponse.json(
  ...
  it("removes the avatar through the binding endpoint", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3100/api/v1/users/me", () =>
  ...
  it("keeps the account page available when logout fails", async () => {
    server.use(
      http.post("http://127.0.0.1:3100/api/v1/auth/logout", () =>
  ...
  it("shows and revokes other active sessions while retaining revoked history", async () => {
    server.use(
      http.get("http://127.0.0.1:3100/api/v1/users/me/sessions", () => HttpResponse.json({
        code: "OK",
        message: "操作成功",
        request_id: "test-request",
        data: {
