...
describe("web HTTP authentication boundary", () => {
  ...
  it.each([
    ...
    let refreshCalls = 0;
    server.use(
      http.all(`http://127.0.0.1:3100${path}`, () => {
        originalCalls += 1;
  ...
  it("replays once with the rotated CSRF cookie", async () => {
    let protectedCalls = 0;
    let refreshCalls = 0;
    document.cookie = "pinjie_blog_web_csrf=old-csrf";
    server.use(
      http.patch("http://127.0.0.1:3100/api/v1/users/me", ({ request }) => {
        protectedCalls += 1;
      ...
      http.post("http://127.0.0.1:3100/api/v1/auth/refresh", () => {
        refreshCalls += 1;
        document.cookie = "pinjie_blog_web_csrf=new-csrf";
        return ok({});
      }),
    );
  ...
  it("expires the session after a definitive refresh rejection", async () => {
    const dispatch = vi.spyOn(window, "dispatchEvent");
    server.use(
      http.get("http://127.0.0.1:3100/api/v1/users/me", () => HttpResponse.json({ code: "AUTH_REQUIRED" }, { status: 401 })),
      http.post("http://127.0.0.1:3100/api/v1/auth/refresh", () =>
