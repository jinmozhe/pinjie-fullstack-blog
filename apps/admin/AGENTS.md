...
- Admin 开发服务使用 `scripts/run-umi.mjs` 通过 `HOST=localhost`、`PORT=3001` 启动；底层 Umi Webpack host 补丁必须保留。不要使用 `max dev --port` 替代项目脚本；直接调用 Umi 时必须从 `apps/admin` 目录运行并显式绑定 `localhost`。
...
- 博客按已确认产品需求引入外围能力；依赖遵守锁定、兼容和验证规则，不因业务扩展放宽准入。
