...
describe("stage C admin workspace", () => {
  ...
  it("creates a user with an administrator supplied initial password", async () => {
    ...
    server.use(
      http.post("http://127.0.0.1:3101/api/v1/admin/users", async ({ request }) => {
  ...
  it("executes user status, credential, and session operations without password confirmation", async () => {
    ...
    server.use(http.patch("http://127.0.0.1:3101/api/v1/admin/users/:id/status", async ({ request }) => {
  ...
  it("selects users and sends one atomic bulk soft-delete request", async () => {
    ...
    server.use(
      http.delete("http://127.0.0.1:3101/api/v1/admin/users/batch", async ({ request }) => {
  ...
  it("loads the recycle bin and sends one atomic bulk restore request", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/users", ({ request }) => {
      ...
      http.post("http://127.0.0.1:3101/api/v1/admin/users/restore/batch", async ({ request }) => {
  ...
  it("loads administrators and assigns roles directly", async () => {
    const user = userEvent.setup();
    server.use(http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({ code: "OK", message: "操作成功", data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 }, request_id: "test-request" })));
  ...
  it("protects administrator status and session operations", async () => {
    ...
    let statusPayload: unknown;
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({
        code: "OK",
        message: "操作成功",
        data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 },
        request_id: "test-request",
      })),
      http.patch("http://127.0.0.1:3101/api/v1/admin/admins/:id/status", async ({ request }) => {
        statusPayload = await request.json();
  ...
  it("edits administrator avatar and display name", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({ code: "OK", message: "操作成功", data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 }, request_id: "test-request" })),
      http.patch("http://127.0.0.1:3101/api/v1/admin/admins/:id", async ({ request }) => {
  ...
  it("switches administrator identity directly", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({ code: "OK", message: "操作成功", data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 }, request_id: "test-request" })),
      http.patch("http://127.0.0.1:3101/api/v1/admin/admins/:id/superuser", async ({ request }) => {
        updatePayload = await request.json();
  ...
  it("keeps superuser grants read-only for ordinary administrators with update permission", async () => {
    ...
    server.use(http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({
      code: "OK",
      message: "操作成功",
      data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 },
      request_id: "test-request",
  ...
  it("resets an administrator password from the more menu", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({ code: "OK", message: "操作成功", data: { items: [otherAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 }, request_id: "test-request" })),
      http.put("http://127.0.0.1:3101/api/v1/admin/admins/:id/credentials/password", async ({ request }) => {
  ...
  it("selects administrators and performs one atomic bulk status request", async () => {
    ...
    let bulkPayload: unknown;
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => HttpResponse.json({ code: "OK", message: "操作成功", data: { items: [current, otherAdmin, thirdAdmin], page: 1, page_size: 20, total: 3, total_pages: 1 }, request_id: "test-request" })),
      http.patch("http://127.0.0.1:3101/api/v1/admin/admins/status/batch", async ({ request }) => {
        bulkPayload = await request.json();
  ...
  it("renders inactive records, empty fields, and active session details", async () => {
    ...
    const ok = (data: unknown) => HttpResponse.json({ code: "OK", message: "操作成功", data, request_id: "test-request" });
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/users", () => ok({ items: [inactiveUser], page: 1, page_size: 20, total: 1, total_pages: 1 })),
      http.get("http://127.0.0.1:3101/api/v1/admin/users/:id/sessions", () => ok({ items: [activeSession, revokedSession], page: 1, page_size: 20, total: 2, total_pages: 1 })),
      http.post("http://127.0.0.1:3101/api/v1/admin/users/:id/sessions/revoke-all", () => ok({ completed: true })),
      http.get("http://127.0.0.1:3101/api/v1/admin/admins", () => ok({ items: [inactiveAdmin], page: 1, page_size: 20, total: 1, total_pages: 1 })),
      http.get("http://127.0.0.1:3101/api/v1/admin/admins/:id/sessions", () => ok({ items: [activeSession, revokedSession], page: 1, page_size: 20, total: 2, total_pages: 1 })),
      http.get("http://127.0.0.1:3101/api/v1/admin/roles", () => ok({ items: [{ id: "01900000-0000-7000-8000-000000000025", code: "auditors", name: "审计员", description: null, is_active: false, permissions: [], created_at: now, updated_at: now }], page: 1, page_size: 100, total: 1, total_pages: 1 })),
      http.patch("http://127.0.0.1:3101/api/v1/admin/roles/:id", async ({ request }) => {
      ...
      http.get("http://127.0.0.1:3101/api/v1/admin/permissions", () => ok([{ id: "01900000-0000-7000-8000-000000000026", code: "users:read", name: "查看用户", description: null, is_active: false, catalog_version: "v1", assignable_to_roles: true }])),
    );
  ...
  it("shows the permission tree directly and supports search and bulk selection", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/permissions", () => HttpResponse.json({ code: "OK", message: "操作成功", data: catalog, request_id: "test-request" })),
      http.put("http://127.0.0.1:3101/api/v1/admin/roles/:id/permissions", async ({ request }) => {
  ...
  it("selects roles and sends one atomic bulk hard-delete request", async () => {
    ...
    let bulkPayload: unknown;
    server.use(
      http.delete("http://127.0.0.1:3101/api/v1/admin/roles/batch", async ({ request }) => {
        bulkPayload = await request.json();
        return HttpResponse.json({
          code: "OK",
          message: "操作成功",
  ...
  it("explains when request metadata logging is disabled", async () => {
    server.use(http.get("http://127.0.0.1:3101/api/v1/admin/system/request-logs", () => HttpResponse.json({ code: "REQUEST_LOG_DISABLED", message: "请求日志功能已关闭", details: null, request_id: "test-request" }, { status: 409 })));
    const user = userEvent.setup();
    renderPage(<SecurityPage />);
    await user.click(screen.getByRole("tab", { name: "请求元数据" }));
  ...
  it("loads file assets and applies filename, scene, and uploader filters", async () => {
    ...
    let lastQuery = "";
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/assets", ({ request }) => {
  ...
  it("previews image assets in place and keeps other files opening in a new tab", async () => {
    const user = userEvent.setup();
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/assets", () => HttpResponse.json({
        code: "OK",
        message: "操作成功",
        data: {
          items: [
  ...
  it("handles compact assets, copy failures, pagination, and single deletion", async () => {
    ...
    server.use(
      ...
      http.delete("http://127.0.0.1:3101/api/v1/assets/:id", ({ params }) => {
  ...
  it("selects assets and sends one atomic bulk hard-delete request", async () => {
    ...
    let bulkPayload: unknown;
    server.use(
      http.delete("http://127.0.0.1:3101/api/v1/assets/batch", async ({ request }) => {
        bulkPayload = await request.json();
        return HttpResponse.json({
          code: "OK",
          message: "操作成功",
