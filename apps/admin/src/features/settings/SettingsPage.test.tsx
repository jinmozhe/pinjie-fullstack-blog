...
describe("SettingsPage", () => {
  ...
  it("loads and saves site settings with the current revision", async () => {
    let payload: unknown;
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => ok(siteSetting)),
      http.patch("http://127.0.0.1:3101/api/v1/admin/settings/site", async ({ request }) => {
  ...
  it("uploads and removes a valid site logo", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => ok(siteSetting)),
      http.delete("http://127.0.0.1:3101/api/v1/admin/settings/site/logo", ({ request }) => {
  ...
  it("preserves the logo and site draft after a failed removal and permits retry", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => ok(siteSetting)),
      http.delete("http://127.0.0.1:3101/api/v1/admin/settings/site/logo", () => {
        attempts += 1;
        return attempts === 1
  ...
  it("requires reloading and fresh confirmation after a logo revision conflict", async () => {
    ...
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => ok(latest)),
      http.delete("http://127.0.0.1:3101/api/v1/admin/settings/site/logo", ({ request }) => {
  ...
  it("keeps edited values on a revision conflict and can load the latest site settings", async () => {
    let reads = 0;
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => {
        reads += 1;
        return ok(reads === 1 ? siteSetting : { ...siteSetting, name: "远端站点", revision: 2 });
      }),
      http.patch("http://127.0.0.1:3101/api/v1/admin/settings/site", () =>
        HttpResponse.json(
  ...
  it("renders site settings as read-only without update permission", async () => {
    server.use(http.get("http://127.0.0.1:3101/api/v1/admin/settings/site", () => ok(siteSetting)));
    renderSettingsPage({
      ...admin,
      is_superuser: false,
  ...
  it("loads and saves the registration policy", async () => {
    let payload: unknown;
    server.use(
      http.patch("http://127.0.0.1:3101/api/v1/admin/settings/registration", async ({ request }) => {
        payload = await request.json();
  ...
  it("handles a registration revision conflict and reloads the current policy", async () => {
    let reads = 0;
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/settings/registration", () => {
        reads += 1;
        return ok({ enabled: reads > 1, revision: reads, updated_at: now, updated_by: null });
      }),
      http.patch("http://127.0.0.1:3101/api/v1/admin/settings/registration", () =>
        HttpResponse.json(
          { code: "SETTINGS_REVISION_MISMATCH", message: "设置已被修改", request_id: "test-request" },
          { status: 412 },
        ),
