...
describe("admin HTTP authentication boundary", () => {
  it("refreshes once after a protected request returns 401 and replays it once", async () => {
    ...
    let refreshCalls = 0;
    document.cookie = "pinjie_blog_admin_csrf=csrf-value";
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/auth/me", () => {
        protectedCalls += 1;
        return protectedCalls === 1 ? HttpResponse.json({ code: "AUTH_REQUIRED", message: "登录已失效" }, { status: 401 }) : ok({ id: "admin" });
      }),
      http.post("http://127.0.0.1:3101/api/v1/admin/auth/refresh", ({ request }) => {
        refreshCalls += 1;
  ...
  it("does not recursively refresh excluded authentication endpoints", async () => {
    let refreshCalls = 0;
    server.use(
      http.post("http://127.0.0.1:3101/api/v1/admin/auth/login", () =>
        HttpResponse.json({ code: "AUTH_INVALID", message: "凭据无效" }, { status: 401 }),
      ),
      http.post("http://127.0.0.1:3101/api/v1/admin/auth/refresh", () => {
        refreshCalls += 1;
        return ok({ session_id: "session" });
      }),
    );
  ...
  it.each(["AUTH_INVALID_CREDENTIALS", "UNKNOWN_ERROR"])("does not refresh a business or unknown 401: %s", async (code) => {
    ...
    let passwordCalls = 0;
    server.use(
      http.post("http://127.0.0.1:3101/api/v1/admin/auth/password", () => {
        passwordCalls += 1;
  ...
  it("adds JSON and CSRF headers to unsafe requests", async () => {
    document.cookie = "pinjie_blog_admin_csrf=csrf%20token";
    server.use(
      http.patch("http://127.0.0.1:3101/api/v1/admin/users/user-id", async ({ request }) => {
  ...
  it("uses a safe fallback for non-JSON failures and exposes retry metadata", async () => {
    server.use(
      http.get("http://127.0.0.1:3101/api/v1/admin/permissions", () =>
