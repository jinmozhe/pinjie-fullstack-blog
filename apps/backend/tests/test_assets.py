...
def _asset_delete_service(*, storage: AsyncMock, assets: list[SimpleNamespace]) -> AssetService:
    ...
    service._assets = SimpleNamespace(  # type: ignore[assignment]
        ...
        is_referenced_by_article=AsyncMock(return_value=False),
    )
    return service
...
async def test_upload_api_rejects_cookie_from_wrong_origin(client, asset_api_service: AsyncMock) -> None:
    client.cookies.set("pinjie_blog_web_access", "web-access-token")
    response = await client.post(
        "/api/v1/assets/upload",
...
@pytest.mark.parametrize(
    ...
    [
        (False, "http://127.0.0.1:3100", "pinjie_blog_web_access", "pinjie_blog_web_csrf"),
        (True, "http://127.0.0.1:3101", "pinjie_blog_admin_access", "pinjie_blog_admin_csrf"),
    ],
)
@pytest.mark.asyncio
