...
class Settings(BaseSettings):
    ...
    project_name: str = Field(default="Pinjie Fullstack Blog Backend", validation_alias="PROJECT_NAME")
    ...
    web_origins: list[str] = Field(
        default_factory=lambda: ["http://127.0.0.1:3100"],
        validation_alias="WEB_ORIGINS",
    )
    admin_origins: list[str] = Field(
        default_factory=lambda: ["http://127.0.0.1:3101"],
    ...
    jwt_issuer: str = Field(
        default="pinjie-fullstack-blog", validation_alias="JWT_ISSUER", min_length=3, max_length=128
    )
