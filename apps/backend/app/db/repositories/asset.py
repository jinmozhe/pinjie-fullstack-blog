...
from app.db.models import Admin, Asset, PostAsset, User
...
class AssetRepository:
    ...
    async def get_by_urls(self, urls: list[str]) -> list[Asset]:
        return list(await self._session.scalars(select(Asset).where(Asset.url.in_(urls)).order_by(Asset.id)))
    ...
    async def is_referenced_by_article(self, asset_id: uuid.UUID) -> bool:
        reference = await self._session.scalar(select(PostAsset.post_id).where(PostAsset.asset_id == asset_id).limit(1))
